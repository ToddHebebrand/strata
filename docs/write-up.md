...
And the cost compounds the moment there is more than one agent. Because files are opaque text, concurrent work on them means branches, worktrees, and merge conflicts — coordination by diff, resolved after the fact by whoever (or whatever) does the merge. In practice, every multi-agent coding setup today converges on the same pattern: one git worktree per agent, then an integration step to reconcile what they did. That layer — not git itself — is what Strata was built to replace. Git keeps history, review, and distribution; the bet is that the *concurrency* machinery is in the wrong substrate. If the codebase is a canonical graph and every change is a typed operation, the system can see what each change touches before it runs — and many agents can share one codebase directly, with no merge step to exist.
...
Everything above was staging — necessary, but staging. Strata was never primarily a bet about making one agent cheaper; the reason to make the codebase a canonical graph with typed operations was always **coordination** — replacing the worktree-and-merge layer that every multi-agent coding setup currently borrows from git. Multiple agents working in one codebase *simultaneously*, no branches, no worktrees, no integration agent merging text, because the substrate infers what each operation touches and schedules around conflicts instead of reconciling them afterward. The single-agent rounds had to come first: they proved the substrate works end-to-end and mapped where structure actually pays. With that established, Phase 6 asked the original question directly — and the baseline it measured against is precisely the incumbent: git worktrees plus an integration agent.

The mechanism is a Rust coordination kernel in front of the existing TypeScript ingest/render/verify pipeline, which stays authoritative. Its storage architecture is built for concurrency rather than adapted to it: the canonical graph is **memory-native** (analysis and scope inference read kernel memory, not per-query SQL), and durability comes from **redb**, an embedded Rust store, where each publication commits the operation, the graph delta, the lifecycle event, the fence, and the generation digest in *one* transaction — there is no reachable state where history and graph disagree, and recovery is snapshot-plus-delta replay, proven byte-exact under injected crashes. The kernel is a sealed single-owner daemon over a Unix socket; clients never open canonical storage. Agents submit **typed operations** (rename, add-parameter, …); the kernel infers each operation's reservation scope from the reference graph — agents never enumerate lock keys — grants deterministic leases, validates candidates against a fresh view, and publishes with fencing. Grouped changes commit only-green-together. If a concurrent publication invalidates the scope an agent decided under, the agent gets a `needs_decision` naming every renamed symbol and re-decides against the fresh view rather than merging text.
