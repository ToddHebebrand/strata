...
| 3 | Rename and an operation touching one of its references are inferred as overlapping. | Feature-gated analyzer coverage proves inferred reference overlap before mutation. | Add a real supported-operation pair, `rename_symbol(greet)` and `add_parameter(greet)`, and prove Rust derives the overlap from Node facts before either client can mutate or publish dirty state. |
