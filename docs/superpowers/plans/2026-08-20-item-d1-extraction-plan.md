# Item D-1 — Extraction and wire parity: implementation plan (v1)
...
```bash
git add packages/coordination-client packages/live-compare pnpm-lock.yaml
git commit -m "feat(coordination-client): extract the typed client into its own package (zod-only deps)"
```
...
**Consumers (all inside `live-compare`, which is a leaf — nothing outside
imports it):** `src/agent.ts:8`, `src/liveAdapter.ts:14`, `src/service.ts:8`
(type-only), `src/tools.ts` (structural), `src/index.ts` (barrel),
`tests/client.test.ts` (25 sites), `tests/serviceHarness.ts:4`, plus the gate
tests that consume the harness.

- [ ] **Step 1.** Add `@strata-code/coordination-client: workspace:*` to
  `live-compare`'s dependencies; re-point every import.
- [ ] **Step 2.** Keep `live-compare/src/index.ts` re-exporting the client
  surface from the new package, so no downstream import path changes in this
  slice.
- [ ] **Step 3.** Move `tests/client.test.ts` into the new package (it is the
  client's behavioral spec and must travel with it). Reconcile the specifier
  convention: `src/*` uses `.js` ESM specifiers while the test imports
  extensionless — pick the one that builds and say so.
- [ ] **Step 4 (the structural-typing hazard).** `tools.ts`'s
  `CoordinationClientApi` (`:106-140`) is satisfied by `CoordinationClient`
  **structurally**, with no `implements` clause, and the two already disagree:
  the interface omits the trailing `deadlineMs` and widens `findDeclarations`'s
  `kind` to `string`. Across a package boundary that silent coupling gets
  easier to break. Add a compile-time assertion (a
  `const _check: CoordinationClientApi = client` style test, or
  `satisfies`) so a drift breaks the build with a clear message instead of at
  a call site.
