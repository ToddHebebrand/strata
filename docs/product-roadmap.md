...
- [ ] **X protocol-usability iteration.** The one live gap: a fresh decision that must rewrite intent content to a renamed symbol. Candidates: surface the current symbol name in `needs_decision` context; sharpen registered fresh-decision prompt guidance. Requires re-qualification and new approvals.
