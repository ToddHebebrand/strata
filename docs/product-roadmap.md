...
- [x] `extract_function` — pull a span of statements into a new function. Full auto-infer (statement index range + name), sync + await, graph-consistent via the materialization prerequisite; rejects control-flow escapes / this / generics / outer reassignment with explicit reasons. (merge `eb35759`, plan `docs/superpowers/plans/2026-05-28-extract-function.md`)
  - **Validation (N=1 paired dogfood, 2026-05-29, decisions.md):** on a one-shot single-site extraction the file-tools baseline **wins** — $0.097 vs $0.117 (substrate +21%), 4 vs 7 tools, 31s vs 63s; both produced correct tsc-clean extractions. Directionally expected: extract is a single text edit where file tools are cheap. The substrate's value is the graph-traceable post-condition (the new function is immediately findable/renamable with resolved edges), so the tool's win must be shown on a **compound** task (extract → then rename/add_parameter the helper across callers — a rename-class bulk op), not a one-shot. Harness: `pnpm --filter @strata/bench dogfood:extract`. The "visibly wins" gate is **not yet met**; compound-task validation deferred to an explicit budgeted run.
