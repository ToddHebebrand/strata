...
## The named next lever — built, keyed-validated, and found invalid as-built: BG-4 triggered (2026-05-16)
...
The operator-keyed re-run was executed (T01/T05/T08 with T03 as the regression guard; `claude-sonnet-4-6`, N=1, $1.52) and, classified from the persisted transcripts, **falsified the gate as built — it did not validate, it triggered a stop.** Root cause: `runCorpusAcceptance` runs the *entire* corpus vitest suite, and the shared seed `examples/medium` deliberately ships a failing test that **is the T05 task's own fail-before fixture**. The gate is therefore structurally unsatisfiable for T01/T03/T08 by the correct task change alone — to reach a green commit the agent must *also* fix the unrelated T05 bug, which the transcripts show it did (T08 verbatim: *"I need to fix isWithinRange in the same transaction"*). Consequences: **BG-4 (T03 regression) triggered** — the proven atomic single-transaction rename now requires a second transaction repairing an unrelated seed bug (12 tools / 2176 tok / 45 s vs. the proven 7–11 / ~1200–1473 / 24–30 s) — and the scorer is contaminated for T03/T08 (their `vitestPassed=1` reflects the agent fixing T05, not clean task signal). BG-1/BG-3 not triggered; BG-2 secondary. The behavioral-gate *concept* is not refuted; the gate is invalid against a corpus that co-locates multiple tasks' fail-before fixtures in one shared suite — it must be **task-scoped**. Full classification and the candidate gate-scope fixes are the newest-first entry in [`decisions.md`](../decisions.md) (2026-05-16, "Keyed behavioral-gate re-run: BG-4 TRIGGERED"), which is authoritative. The deeper redesign of the gate's scope is the open next lever; a second, untouched lever also remains: evaluate a stronger model at the 11-tool multi-decision surface.
