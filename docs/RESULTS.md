...
- **HN-2 = 2/3 (honest "noted variance" per the frozen scale).** All three T08 trials are process-identical, clean, single-transaction, no cross-task collateral; 2/3 meet T08's strict text criteria. The 1 miss committed a `tsc`-clean, test-passing, contamination-free change that didn't match the regex criteria shape — criteria strictness on a correct change, not a regression or contamination.

Net: the task-scoped gate and the flagship T03 claim are **robust at N=3**; T05 is a robust gate-driven *correctness* win (not efficiency); T08 is a real win with honest 2/3 task-criteria variance; T01 is firmly isolated to the known non-gate lever. Authoritative: the newest-first [`decisions.md`](../decisions.md) entry (2026-05-16, "N=3 hardening: HN-1 PASS … HN-2 = 2/3"). Remaining open levers: (a) the small separable T08 2/3 question (agent solution-shape vs over-strict regex criteria); (b) the now-isolated T01 lever (stronger model and/or `add_parameter` legibility redesign — not prompt tuning, not another gate pass).
