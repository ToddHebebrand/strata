...
const USAGE =
  ...
  "  ANTHROPIC_API_KEY.\n" +
  "\n" +
  `  Default prompt: "${MOVE_DOGFOOD_PROMPT}"\n`;
...
async function main(argv: string[]): Promise<number> {
  ...
  if (!process.env.ANTHROPIC_API_KEY && !process.env.CLAUDE_CODE_OAUTH_TOKEN) {
    process.stderr.write(
      "move dogfood requires ANTHROPIC_API_KEY (or CLAUDE_CODE_OAUTH_TOKEN).\n"
    );
    return 1;
  }

  const result = await runDogfoodMove({
  ...
  const markdown = renderDogfoodMoveMarkdown(result);
  process.stdout.write(markdown);

  if (parsed.outDir) {
    mkdirSync(parsed.outDir, { recursive: true });
    const stamp = new Date().toISOString().replace(/[:.]/g, "-");
    const mdPath = path.join(parsed.outDir, `dogfood-move-${stamp}.md`);
    const jsonPath = path.join(parsed.outDir, `dogfood-move-${stamp}.json`);
...
main(process.argv.slice(2)).then(
  ...
  (err) => {
    process.stderr.write(`dogfood-move failed: ${err?.stack ?? err}\n`);
    process.exit(1);
  }
);
