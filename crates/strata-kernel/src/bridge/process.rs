...
impl NodeBridgeConfig {
    ...
    /// Opts the kernel into the persistent bridge (service flag
    /// `--persistent-bridge`). Default off; the one-shot transport is
...
impl NodeBridgeClient {
    ...
    /// by the persistent bridge router (`router.rs`) so persistent-path
...
/// Shared with the persistent bridge router so both transports measure
